        ...
        public ResponseEntity<Map<String, String>> login(@Valid @RequestBody LoginDTO loginDTO) {
                ...
                try {
                        ...
                        if (authentication.isAuthenticated()) {
                                ...
                                // Obtener permisos del usuario (IDs de permisos) a través de UsuarioRol
                                List<Long> permisos = usuarioRolRepository.findByUsuario(usuario).stream()
                                                .flatMap(usuarioRol -> usuarioRol.getRol().getRolPermisos().stream())
                                                .filter(RolPermiso::getHabilitado)
                                                .map(rolPermiso -> rolPermiso.getPermiso().getId())
                                                .distinct()
                                                .collect(Collectors.toList());

                                // Obtener roles del usuario a través de UsuarioRol
